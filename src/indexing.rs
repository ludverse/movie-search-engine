...
pub fn create_index(stop_words: &Vec<String>, silent: bool) {
    if !silent {
        println!("indexing entries for you on first start up... please hang on tight");
    }
    ...
    let mut progress_bar = BernoulliBar::with_goal(entries.len()).timed();
    ...
    for (_i, entry) in entries.iter().enumerate() {
        ...
        let entry_extract = entry.get("extract").and_then(|extract| extract.as_str()).map(|extract| extract.to_string());

        let tokens = tokenize_term(&entry_title);
    ...
    if !silent {
        println!("done! index saved to index.json and ready!");
        println!();
    }
}
